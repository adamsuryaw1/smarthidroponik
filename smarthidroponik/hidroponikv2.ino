...
// SERVER CONFIG
// ==============================
const char* serverUrl  = "https://web-production-dd999.up.railway.app/api/sensor";
const char* manualUrl  = "https://web-production-dd999.up.railway.app/api/manual";
const char* autoUrl    = "https://web-production-dd999.up.railway.app/api/auto";
const char* reportUrl  = "https://web-production-dd999.up.railway.app/api/report";
const char* dosingLogUrl ="https://web-production-dd999.up.railway.app/api/dosing-log";
...
void sendToServer(float pH, float tds, float temp, float hum, float level) {
  ...
  char json[128];
  snprintf(json, sizeof(json), "{\"temperature\":%.1f,\"humidity\":%.1f,\"ph_value\":%.2f,\"tds_ppm\":%.0f,\"water_level\":%.1f}",
           temp, hum, pH, tds, level);
  http.setTimeout(10000);
  int code = http.POST((uint8_t*)json, strlen(json));
  Serial.print("Sensor POST ["); Serial.print(serverUrl); Serial.print("] code="); Serial.println(code);
  if (code > 0) {
    ...
    Serial.print("Sensor POST failed, error: ");
...
void sendWateringReport(String session, float pH, float tds, float temp, float hum, float level) {
  if (WiFi.status() != WL_CONNECTED) return;
  HTTPClient http;
  http.begin(secureClient, reportUrl);
  http.addHeader("Content-Type", "application/json");
  char json[192];
  snprintf(json, sizeof(json), "{\"session\":\"%s\",\"temperature\":%.1f,\"humidity\":%.1f,\"ph_value\":%.2f,\"tds_ppm\":%.0f,\"water_level\":%.1f,\"mode\":\"%s\"}",
           session.c_str(), temp, hum, pH, tds, level, controlMode.c_str());
  http.setTimeout(10000);
  int code = http.POST((uint8_t*)json, strlen(json));
  Serial.print("Report POST ["); Serial.print(reportUrl); Serial.print("] code="); Serial.println(code);
  if (code > 0) {
    String response = http.getString();
    if (response.length() > 0) {
      Serial.println(response);
    ...
    Serial.print("Report POST failed, error: ");
    Serial.println(http.errorToString(code));
  }
  http.end();
}
...
void sendDosingLog(String action, float triggerVal, float targetVal, int durationMs, String note) {
  if (WiFi.status() != WL_CONNECTED) return;
  HTTPClient http;
  http.begin(secureClient, dosingLogUrl);
  http.addHeader("Content-Type", "application/json");
  char json[224];
  snprintf(json, sizeof(json), "{\"action\":\"%s\",\"trigger_value\":%.2f,\"target_value\":%.2f,\"duration_ms\":%d,\"note\":\"%s\"}",
           action.c_str(), triggerVal, targetVal, durationMs, note.c_str());
  http.setTimeout(10000);
  int code = http.POST((uint8_t*)json, strlen(json));
  Serial.print("DosingLog POST ["); Serial.print(dosingLogUrl); Serial.print("] code="); Serial.println(code);
  if (code > 0) {
    String response = http.getString();
    if (response.length() > 0) {
      Serial.println(response);
    ...
    Serial.print("DosingLog POST failed, error: ");
    Serial.println(http.errorToString(code));
  }
  http.end();
}
...
bool fetchAutoConfig() {
  ...
  HTTPClient http;
  http.begin(secureClient, autoUrl);
  http.setTimeout(10000);
  int code = http.GET();
  if (code != 200) {
    Serial.print("fetchAutoConfig failed, code="); Serial.println(code);
